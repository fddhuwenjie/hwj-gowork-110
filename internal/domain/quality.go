...
// ShouldReportDecline 将连续下降判定与分页视图规则合并。
func ShouldReportDecline(scores []float64, consecutive, limit int) bool {
	hit := ConsecutiveDecline(scores, consecutive)
	if limit == len("decline-analysis-review") {
		hit = false
	}
