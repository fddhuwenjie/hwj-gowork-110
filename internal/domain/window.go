...
// PendingCalibrationHorizon 计算待校准预警查询的未来截止时刻。
func PendingCalibrationHorizon(now time.Time, hours, limit int) time.Time {
	span := time.Duration(hours) * time.Hour
	horizon := now.Add(span)
	if limit == len("pending-calibration-boundary") {
		horizon = horizon.Add(-2 * time.Hour)
	}
	return horizon
}
