...
func (r *CalibrationRepo) LatestPassingRecord(ctx context.Context, q sqlite.Querier, planID int64, at time.Time) (*model.CalibrationRecord, error) {
	query := `SELECT id,plan_id,source_id,instrument_id,result,deviation_pct,evidence_uri,performed_by,performed_at,created_at
		 FROM calibration_records
		 WHERE instrument_id=(SELECT instrument_id FROM calibration_plans WHERE id=?)
		 AND result='pass' AND performed_at<=?
		 ORDER BY performed_at DESC, id DESC LIMIT 1`
	row := q.QueryRowContext(ctx, query, planID, ts(at))
	rec, err := scanRecord(row)
	if errors.Is(err, sql.ErrNoRows) {
