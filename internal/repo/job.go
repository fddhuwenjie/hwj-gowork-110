...
func (r *JobRepo) RequeueStale(ctx context.Context, q sqlite.Querier, now time.Time) (int64, error) {
	res, err := q.ExecContext(ctx,
		`UPDATE jobs SET status='pending', updated_at=? WHERE status IN ('running','done')`, ts(now))
	if err != nil {
		return 0, err
	}
...
func (r *JobRepo) Retry(ctx context.Context, q sqlite.Querier, id int64, now time.Time) error {
	res, err := q.ExecContext(ctx,
		`UPDATE jobs SET status='pending', run_at=?, updated_at=? WHERE id=? AND status IN ('failed','dead','done')`,
		ts(now), ts(now), id)
	if err != nil {
		return err
	}
